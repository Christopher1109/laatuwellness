...
-- storage bucket para fotos de check-in de staff
...
create policy "staff_photos_read" on storage.objects for select to authenticated
  using (bucket_id = 'staff-photos' and (owner = auth.uid() or public.has_role(auth.uid(),'admin')));
create policy "staff_photos_write" on storage.objects for insert to authenticated
...
-- ----------------------------------------------------------------------------
-- NÓMINA: ajustes (bonos, descuentos, penalización por no-show, etc.)
-- El total de horas trabajadas se calcula a partir de time_clock_entries
-- y la tarifa de staff_profiles.hourly_rate_cents; payroll_adjustments cubre
-- los casos especiales (falta de check-in, extras, descuentos).
-- ----------------------------------------------------------------------------
create table public.payroll_adjustments (
  id uuid primary key default gen_random_uuid(),
  staff_id uuid not null references public.staff_profiles(id) on delete cascade,
  amount_cents int not null, -- positivo = bono/extra, negativo = descuento/penalización
...
-- helper: horas trabajadas en un rango, emparejando entradas/salidas por orden
...
-- marca no-show automático y penaliza en nómina cuando aplica (llamado manual desde el panel)
...
declare _row public.check_ins; _class_coach uuid;
begin
...
-- venta de mostrador: descuenta stock de producto automáticamente
...
-- ----------------------------------------------------------------------------
-- SEED: staff de ejemplo (20 perfiles) — se completan luego con user_id real
-- cuando cada persona inicia sesión por primera vez con su correo administrativo
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- LOGIN INTELIGENTE: al registrarse/entrar por primera vez con un correo
-- que ya está dado de alta en staff_profiles, se vincula automáticamente
-- ese usuario como staff/coach/admin y se le otorga el rol correspondiente
-- en user_roles (además de 'user'), para que el login lo mande a la zona
-- administrativa en lugar de la zona de cliente.
-- ----------------------------------------------------------------------------
...
-- por si el usuario administrativo ya tenía cuenta creada antes de esta migración
